...
if ! command -v omp >/dev/null 2>&1; then
  print -u2 "omp was not found in PATH. This script configures an existing installation; it does not install omp."
  exit 1
fi

project_root="$(cd -- "$project_arg" && pwd -P)"
if [[ "$project_root" == "/" || "$project_root" == "$HOME" ]]; then
  print -u2 "Refusing to configure a home or filesystem-root directory. Pass a project directory."
  exit 1
fi
...
write_managed() {
  ...
  if [[ -f "$destination" ]] && cmp -s -- "$source_file" "$destination"; then
    print "Unchanged: ${destination:t}"
    return
  fi
  if [[ -f "$destination" ]] && ! grep -qF "$marker" "$destination"; then
    if [[ "$force" -ne 1 ]]; then
      cp -- "$source_file" "$destination.recommended"
      print -u2 "Kept existing $destination; review $destination.recommended or rerun with --force."
      return
    fi
    cp -p -- "$destination" "$destination.bak.$timestamp"
  fi
  cp -- "$source_file" "$destination"
  print "Wrote: ${destination:t}"
}
...
print "OMP project bootstrap complete. Review .omp/, then start a new session in this project."
