...
# dd network speed + stability test for Ubuntu 26.04.
# The remote endpoint must be a host you own or have permission to test.
# Bytes are discarded by /dev/null; no test file is created.
...
usage() {
  cat <<'EOF'
Usage:
  dd-network-test-ubuntu2604.sh --remote-host HOST [options]
...
Examples (replace placeholders):
  ./dd-network-test-ubuntu2604.sh --remote-host 'test-endpoint' --remote-user 'testuser'
  SIZE_MIB=64 ROUNDS=3 ./dd-network-test-ubuntu2604.sh --remote-host 'test-endpoint'
EOF
}
...
while (($#)); do
  case "$1" in
    --remote-host) [[ $# -ge 2 ]] || die "--remote-host needs a value"; REMOTE_HOST="$2"; shift 2 ;;
    --remote-user) [[ $# -ge 2 ]] || die "--remote-user needs a value"; REMOTE_USER="$2"; shift 2 ;;
    --port) [[ $# -ge 2 ]] || die "--port needs a value"; PORT="$2"; shift 2 ;;
    --size-mib) [[ $# -ge 2 ]] || die "--size-mib needs a value"; SIZE_MIB="$2"; shift 2 ;;
    --rounds) [[ $# -ge 2 ]] || die "--rounds needs a value"; ROUNDS="$2"; shift 2 ;;
    --direction) [[ $# -ge 2 ]] || die "--direction needs a value"; DIRECTION="$2"; shift 2 ;;
...
# A pipe may return short reads. fullblock makes count mean the requested
# number of complete blocks instead of “number of reads that happened”.
...
if ssh "${SSH_OPTS[@]}" "$SSH_TARGET" 'command -v dd >/dev/null 2>&1'; then
  echo "PASS"
else
  die "remote endpoint does not provide dd or SSH authentication failed"
fi
...
run_direction() {
  local direction="$1" round
  for ((round=1; round<=ROUNDS; round++)); do run_one "$direction" "$round"; done
}
...
awk -F '\t' -v size="$SIZE_MIB" '
  { total++; if ($2 == 0) { n[$1]++; r=$3>0 ? size/$3 : 0; sum[$1]+=r; sumsq[$1]+=r*r; if (!( $1 in min) || r<min[$1]) min[$1]=r; if (!( $1 in max) || r>max[$1]) max[$1]=r } else fail[$1]++ }
  END { for (d in n) { avg=sum[d]/n[d]; variance=sumsq[d]/n[d]-avg*avg; if (variance<0) variance=0; cv=avg>0 ? sqrt(variance)/avg*100 : 0; printf "SUMMARY %s samples=%d avg=%.1f MiB/s min=%.1f max=%.1f CV=%.1f%% failures=%d\n", d,n[d],avg,min[d],max[d],cv,fail[d]+0 } }
' "$RESULTS"

if awk -F '\t' '$2 != 0 { bad=1 } END { exit bad ? 0 : 1 }' "$RESULTS"; then
  exit 1
fi
echo "Completed without transfer failures. Repeat at different times if you need a stability baseline."
