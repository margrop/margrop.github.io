...
command -v git >/dev/null || { command -v brew >/dev/null || { echo "请先安装 Homebrew"; exit 1; }; brew install git; }
command -v node >/dev/null || { command -v brew >/dev/null || { echo "请先安装 Homebrew"; exit 1; }; brew install node; }

read -r -p "Proxmox 地址（域名或脱敏后的实际地址，不含 https://）: " PROXMOX_HOST
...
echo "完成。重启 AI Agent 后，先问：只读列出 Proxmox 节点和虚拟机，不要执行任何写操作。"
