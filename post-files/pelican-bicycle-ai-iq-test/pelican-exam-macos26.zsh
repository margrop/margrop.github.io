...
# 鹈鹕骑车测试 · 离线阅卷器 (macOS 26 一键版, 零依赖, 仅用系统 python3 标准库)
# 与 Ubuntu 版共享同一 Python 内核, 仅启动器不同, 并在阅卷后用 Preview 打开原图
# 用法: ./pelican-exam-macos26.zsh my-answer.svg   (无参数则生成答题模板)
setopt errexit nounset pipefail no_undef

if ! command -v python3 >/dev/null 2>&1; then
  echo "未找到 python3, 请先: xcode-select --install" >&2
  exit 1
fi
...
TPL
  ...
  echo "把 AI 给你的 SVG 代码粘贴进去保存, 再运行: ./pelican-exam-macos26.zsh my-answer.svg"
  exit 0
fi
...
# macOS 专属福利: 阅卷完顺手用 Preview 打开原图, 人眼复核几何检查看不出的问题
if [[ -f "${1:-}" ]]; then
